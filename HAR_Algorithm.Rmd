---
title: "Human Activity Recognition Algorithm"
output:
  html_document:
    keep_md: yes
    toc: yes
...
```{r removeNA, cache=TRUE,  echo = TRUE}
...
```{r partitiondata, cache=TRUE,  echo = TRUE}
set.seed(4543)
...
```{r removetimestamp, cache=TRUE,  echo = TRUE}
training <- training[,-c(1,2,3,4,5,6)]
```

5. Remove features  with near zero variance and are not good predictors.
```{r removeNearZeroFrequency, cache=TRUE,  echo = TRUE}
...
```{r applyTransformations, cache=TRUE,  echo = TRUE}
...
```{r traincontrol, cache=TRUE,  echo = TRUE}
...
```{r gridControl, cache=TRUE,  echo = TRUE}
...
```{r modelTuning, cache=TRUE,  results='hide', echo = TRUE}
...
```{r accuracy, cache=TRUE,  results='asis', echo = TRUE}
...
```{r outofSampleError, cache=TRUE,  results='asis', echo = TRUE}
...
```{r comparison, cache=TRUE,  results='asis', echo = TRUE}
...
```{r plotFigure1, fig.height=6,fig.width=8 }
# resampleHist(gbmFit)
```
